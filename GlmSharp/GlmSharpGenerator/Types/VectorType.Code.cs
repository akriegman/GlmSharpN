...
namespace GlmSharpGenerator.Types
{
    partial class VectorType
    {
        ...
        public override IEnumerable<Member> GenerateMembers()
        {
            ...
            {
                yield return new Property(Program.CDIMS[c].ToString(), BaseType)
                {
                    GetterLine = $"return {Program.DIMS[c]};",
                    SetterLine = $"{Program.DIMS[c]} = value;",
            ...
            for (var comps = 2; comps <= Program.N; ++comps)
            {
                var commentAddition = "";
                if (comps < Components)
                ...
                for (var ucomps = comps; ucomps < Components; ++ucomps)
                {
                    ...
                    yield return new Constructor(this, Fields)
                    {
                        ParameterString = new VectorType(BaseType, comps).NameThat + " ve, " + SubCompParameterString(comps, ucomps),
                        Initializers = "ve".DotComp(comps).Concat(SubCompArgs(comps, ucomps)),
