...
namespace GlmSharpGenerator.Types
{
    partial class SwizzleType
    {
            ...
            get
            {
                foreach (var swizzle in Swizzle(0))
                {
                    ...
                    for (var i = 0; i < swizzle.Length; ++i)
                        yield return $"Assert.AreEqual({vals[Program.DIMS.IndexOf(swizzle[i])]}, v.{Program.DIMS[i]});".Indent();
                    yield return "}";
                }
            }
        }
        ...
        private IEnumerable<string> InlineXyzwTests
        {
            get
            {
                foreach (var swizzleBits in InlineSwizzle())
                {
                    ...
                    var swizzle = swizzleBits.Select((c, i) => c == '1' ? Program.DIMS[i].ToString() : "").Aggregate((s1, s2) => s1 + s2);
                    ...
                    for (var i = 0; i < Components; ++i)
                        yield return $"Assert.AreEqual({(swizzleBits[i] == '1' ? subvals[swizzleBits.Substring(0, i).Count(c => c == '1')] : vals[i])}, v0.{Program.DIMS[i]});".Indent();
                    yield return "";
                    for (var i = 0; i < swizzle.Length; ++i)
                        yield return $"Assert.AreEqual({vals[Program.DIMS.IndexOf(swizzle[i])]}, v2.{Program.DIMS[i]});".Indent();
                    yield return "}";
                }
            }
        }
        ...
        private IEnumerable<string> InlineRgbaTests
        {
            get
            {
                foreach (var swizzleBits in InlineSwizzle())
                {
                    ...
                    yield return "";
                    if (swizzle.Length == 1)
                        yield return $"Assert.AreEqual({vals[Program.DIMS.IndexOf(swizzle[0])]}, v2);".Indent();
                    else
                        for (var i = 0; i < swizzle.Length; ++i)
                            yield return $"Assert.AreEqual({vals[Program.DIMS.IndexOf(swizzle[i])]}, v2.{Program.DIMS[i]});".Indent();
                    yield return "}";
        ...
        {
            yield return new TestFunc(this, Program.DIMS, XyzwTests);
            yield return new TestFunc(this, Program.CDIMS, RgbaTests);
