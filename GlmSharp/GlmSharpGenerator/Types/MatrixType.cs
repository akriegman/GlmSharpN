...
namespace GlmSharpGenerator.Types
{
    class MatrixType : AbstractType
    {
        ...
        public string ArgOf(int c) => Program.DIMS[c].ToString();
        ...
        public override IEnumerable<Member> GenerateMembers()
        {
            ...
            // by-mat-ctors
            for (var rows = 2; rows <= Program.N; ++rows)
                for (var cols = 2; cols <= Program.N; ++cols)
                {
            ...
            if (BaseType.IsFloatingPoint && Rows == Columns && Rows >= 3 && Rows <= 4)
            {
                yield return new Constructor(this, Fields)
                {
        ...
        protected override IEnumerable<string> Body
        {
            get
            {
                ...
                if (BaseType.HasArithmetics)
                {
                    ...
                    for (var rcols = 2; rcols <= Program.N; ++rcols)
                    {
                        var lhsRows = Rows;
                        var lhsCols = Columns;
                        var rhsRows = Columns;
                    ...
                    yield return string.Format("public static {0} operator*({1} m, {2} v) => new {0}({3});", BaseType.Prefix + "vec" + Rows, NameThat, BaseType.Prefix + "vec" + Columns,
                        Rows.ForIndexUpTo(r => Columns.ForIndexUpTo(c => "m.m" + c + r + " * v." + Program.DIMS[c]).Aggregated(" + ")).CommaSeparated());
