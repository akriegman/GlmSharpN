...
namespace GlmSharpGenerator.Types
{
    class QuaternionType : AbstractType
    {
        ...
        public IEnumerable<string> Fields => Program.DIMS.Substring(0, Components).Select(c => c.ToString());


        public override string Folder => "Quat";

        public string CompString => Program.DIMS.Substring(0, Components);
        ...
        public char ArgOf(int c) => Program.DIMS[c];
        public string ArgOfs(int c) => Program.DIMS[c].ToString();
        public char ArgOfUpper(int c) => char.ToUpper(Program.DIMS[c]);

        public IEnumerable<string> SubCompParameters(int start, int end) => Program.DIMS.Substring(start, end - start + 1).Select(c => BaseTypeName + " " + c);
        public string SubCompParameterString(int start, int end) => SubCompParameters(start, end).CommaSeparated();
        public IEnumerable<string> SubCompArgs(int start, int end) => Program.DIMS.Substring(start, end - start + 1).Select(c => c.ToString());
